...
4-corner colored blocks (inner/mid/outer). Color scale: `#1e293b` (cold/no data) → blue → green → yellow → red (hot). Header reads "PIT TYRE TEMPS" (hover tooltip explains why): iRacing only exposes carcass temps via the SDK, so values change slowly while driving and are most visible during pit stops. No auto-hide — every car exposes carcass temps so the overlay is always meaningful when enabled.
